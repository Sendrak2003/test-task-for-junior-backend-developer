...
## Приведение проекта в рабочее состояние

В исходном проекте отсутствовала часть конфигурации для локального запуска.

В рамках выполнения задания были внесены изменения:
- добавлен `.env.example` с необходимыми переменными окружения
- настроены и исправлены переменные окружения и DSN подключения к БД
- обновлён `docker-compose.yml` для корректного запуска сервисов
- дополнен `.gitignore`

Это позволило обеспечить стабильный запуск проекта через Docker и локально..
